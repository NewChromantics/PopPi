...
{
public:
	TCanvas(std::function<void()> Unlock) :
		mUnlock	( Unlock )
	{
	}
	...
	uint32_t				GetIndex(int x,int y) const	{	return x + (y*mWidth);	}
...
public:
	...
	void		FillRow(int y,TPixel Colour);
	void		FillPixels(TPixel Colour);
	void		FillPixelsCheckerBoard(int SquareSize,TPixel ColourA,TPixel ColourB);
	...
	uint32_t	GetLineSpacing() const	{	return 2;	}
	
private:
	TCanvas<TPixel>	GetCanvas();
